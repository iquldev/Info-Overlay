...
public class OverlayRenderer {
    ...
    private static void renderMediaWidget(GuiGraphicsExtractor context, Minecraft client, iquldev.fpsoverlay.stats.MediaStats media, Position pos, int bgColor, int textColor, int hPadding, int vPadding, int rounding) {
        iquldev.fpsoverlay.stats.MediaStats.MediaInfo info = media.getInfo();
        int fontHeight = client.font.lineHeight;
        int iconW = client.font.width("▶");
        ...
        if (Util.getPlatform() == Util.OS.OSX) {
            int musicIconW = client.font.width("🎵");
            int rowTextX = pos.x() + musicIconW + MEDIA_ICON_TEXT_GAP;
            String title = ellipsizeUnicode(Component.translatable("fpsoverlay.media.unsupported_os").getString(), lineMax);
            String artist = ellipsizeUnicode(Component.translatable("fpsoverlay.media.unsupported_os_mac").getString(), lineMax);
            int contentW = Math.max(client.font.width(title), client.font.width(artist));
            ...
            context.text(client.font, "🎵", pos.x(), iconY, textColor, false);
            context.text(client.font, title, rowTextX, pos.y(), textColor, false);
            context.text(client.font, "§7" + artist, rowTextX, pos.y() + fontHeight + MEDIA_LINE_GAP, textColor, false);
            return;
        }
        ...
        if (info.isEmpty()) {
            int musicIconW = client.font.width("🎵");
            int rowTextX = pos.x() + musicIconW + MEDIA_ICON_TEXT_GAP;
            String title = info.title().isEmpty() ? Component.translatable("fpsoverlay.media.widget_name").getString() : info.title();
            String artist = info.artist().isEmpty() ? Component.translatable("fpsoverlay.media.not_playing").getString() : info.artist();
        ...
        if (InfoOverlayConfig.mediaWidgetCompact) {
            ...
            int contentW = client.font.width(line) + client.font.width("§8" + suffix);
            int totalW = iconW + MEDIA_ICON_TEXT_GAP + contentW;
            String transportIcon = info.isPlaying() ? "▶" : "||";
            int y2 = pos.y() + fontHeight + vPadding;
            drawRoundedRect(context, pos.x() - hPadding, pos.y() - vPadding, pos.x() + totalW + hPadding, y2, rounding, bgColor);
            context.text(client.font, transportIcon, pos.x(), pos.y(), textColor, false);
            context.text(client.font, line, textX, pos.y(), textColor, false);
            context.text(client.font, "§8" + suffix, textX + client.font.width(line), pos.y(), textColor, false);
            return;
        }

        int titleW = client.font.width("§l" + title);
        int artistW = client.font.width("§7" + artist);
        int timeW = client.font.width("§7" + time);
        ...
        String transportIcon = info.isPlaying() ? "▶" : "||";
        context.text(client.font, transportIcon, pos.x(), pos.y(), textColor, false);
        context.text(client.font, "§l" + title, textX, pos.y(), textColor, false);
        context.text(client.font, "§7" + artist, textX, pos.y() + fontHeight + MEDIA_LINE_GAP, textColor, false);
        ...
        context.text(client.font, "§7" + time, textX, barRowTop + MEDIA_BAR_HEIGHT + MEDIA_AFTER_BAR, textColor, false);
    }
    ...
    private static int measurePlaceholderContentWidth(Minecraft client, String text, iquldev.fpsoverlay.stats.MediaStats media, boolean isMedia) {
        if (!isMedia) {
        ...
        if (Util.getPlatform() == Util.OS.OSX) {
            int musicIconW = client.font.width("🎵");
            int w = Math.max(
                    client.font.width(ellipsizeUnicode(Component.translatable("fpsoverlay.media.unsupported_os").getString(), lineMax)),
                    client.font.width(ellipsizeUnicode(Component.translatable("fpsoverlay.media.unsupported_os_mac").getString(), lineMax))
            );
        ...
        if (info.isEmpty()) {
            ...
            String artist = info.artist().isEmpty() ? Component.translatable("fpsoverlay.media.not_playing").getString() : info.artist();
            int w = Math.max(
                    client.font.width(ellipsizeUnicode(title, lineMax)),
                    client.font.width(ellipsizeUnicode(artist, lineMax))
            );
            return musicIconW + gap + w;
        }
        if (InfoOverlayConfig.mediaWidgetCompact) {
            String core = info.title().isEmpty() ? info.artist() : (info.artist().isEmpty() ? info.title() : info.title() + " · " + info.artist());
            int prefix = client.font.width(ellipsizeUnicode(core, compactMax));
            int suffix = client.font.width("§8" + MEDIA_TIME_COMPACT_SAMPLE);
            return iconW + gap + prefix + suffix;
        }
        int tw = client.font.width("§l" + ellipsizeUnicode(info.title(), lineMax));
        int aw = client.font.width("§7" + ellipsizeUnicode(info.artist(), lineMax));
        int timeW = client.font.width(MEDIA_TIME_WIDTH_SAMPLE);
        return iconW + gap + Math.max(Math.max(tw, aw), timeW);
    }

    private static int measurePlaceholderContentHeight(Minecraft client, String text, iquldev.fpsoverlay.stats.MediaStats media, int fontHeight, boolean isMedia) {
        if (!isMedia) {
            return fontHeight;
        }
        iquldev.fpsoverlay.stats.MediaStats.MediaInfo info = media.getInfo();
        if (InfoOverlayConfig.mediaWidgetCompact && !info.isEmpty() && Util.getPlatform() != Util.OS.OSX) {
            return fontHeight;
        }
        if (Util.getPlatform() == Util.OS.OSX || info.isEmpty()) {
